LOAD DATA LOCAL INFILE '5letter6hop.csv' 
INTO TABLE pair5
...
delete from pair5 where pair = 'pair';

update pair5 p_longer
set keep = 1
where not exists(
  select 1
  from pair5 p_shorter
...
INTO OUTFILE '/var/lib/mysql/5_toload_6.csv'
FIELDS TERMINATED BY ','
LINES TERMINATED BY '\n'
from pair5
where keep = 1
...
INTO OUTFILE '/var/lib/mysql/5_pairs_uniq.csv'
FIELDS TERMINATED BY ','
LINES TERMINATED BY '\n'
from pair5
where keep = 1
...
set @rn = 0;
LOAD DATA LOCAL INFILE 'pairs_5_6.txt' 
INTO TABLE allpairs
FIELDS TERMINATED BY ',' 
optionally enclosed BY '"'
LINES TERMINATED BY '\n'
(pair) set letters_hops=56, pairoffset= (@rn := @rn + 1)
;
